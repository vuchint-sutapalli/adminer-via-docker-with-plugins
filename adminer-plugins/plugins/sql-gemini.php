...
class AdminerSqlGemini extends Adminer\Plugin {
	...
	*/
	function __construct($apiKey = 'AIzaSyDWDbPjmvH9_hphsnY_yJGdue42qRMG3do', $model = "gemini-2.5-flash") {
		error_log("--- Gemini Plugin: Constructor Called Successfully ---"); // <-- ADD THIS LOG
		$this->apiKey = $apiKey;
		$this->model = $model;
	}
	...
	function sqlPrintAfter() {
		error_log("--- Gemini Plugin: UI Rendering (sqlPrintAfter) Started ---"); // <-- ADD THIS LOG
...
<?php
error_log("--- Gemini Plugin: UI Rendering (sqlPrintAfter) Finished ---"); // <-- ADD THIS LOG
	}
